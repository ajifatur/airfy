...
<title>Login – {{ $var_kontak->nama_perusahaan }}</title>
				...
				<form id="form" method="post" action="/login">
					...
					<div class="input-group form-group mb-0">
						<div class="input-group-prepend">
							<span class="input-group-text {{ $errors->has('username') ? 'bg-danger' : '' }}"><i class="fas fa-user"></i></span>
						</div>
						<input type="text" name="username" class="form-control {{ $errors->has('username') ? 'border-danger' : '' }}" id="username" placeholder="Username">
					</div>
					@if($errors->has('username'))
					<small class="text-danger">{{ ucfirst($errors->first('username')) }}</small>
					@endif
					<div class="input-group form-group mb-0 mt-2">
						<div class="input-group-prepend">
							<span class="input-group-text {{ $errors->has('password') ? 'bg-danger' : '' }}"><i class="fas fa-key"></i></span>
						</div>
					...
					<div class="form-group mt-2">
						...
						<button type="submit" id="login" class="btn float-right login_btn">Login</button>
					</div>
				</form>
			</div>
